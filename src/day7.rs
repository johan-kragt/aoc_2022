...
pub fn day7() {
    ...
    if let Ok(lines) = read_lines(format!("./data/day{}.txt", day)) {
        ...
        let mut current_dir = Rc::new(RefCell::new(Dir::new(String::from("/"))));
                ...
                match &ip[0..4] {
                    "$ cd" => {
                        ...
                        match &ip[5..] {
                            ...
                            _ => {
                                let rc = Rc::new(RefCell::new(Dir::new(String::from(&ip[5..]))));
        ...
        println!("{}", root.borrow().report_dirs_under(100000).to_string().bright_white());
        let vec = root.borrow().list_dirs_freeing_more_then(30000000 - (70000000 - root.borrow().size()));
...
impl Dir {
    ...
    pub fn set_parent(&mut self, node: Rc<RefCell<Dir>>) {
    ...
    pub fn report_dirs_under(&self, limit: u32) -> u32 {
        let i = self.children.iter().map(|f| f.borrow().report_dirs_under(limit)).sum::<u32>();
        if &self.size() < &limit {
            return i + &self.size();
        } else {
            return i
    ...
    pub fn list_dirs_freeing_more_then(&self, limit: u32) -> Vec<u32> {
        ...
        self.children.iter()
            .for_each(|x| x.borrow().list_dirs_freeing_more_then(limit).iter().for_each(|y| result.push(*y)));
        if(self.size() > limit) {
