...
pub fn day4() {
    ...
    if let Ok(lines) = read_lines(format!("./data/day{}.txt", day)) {
        ...
        let count_fully_contains = assignment_pair.iter()
            .filter(|p| p.full_overlap())
            .count();
        println!("{}", count_fully_contains.to_string().bright_white());

        let count_partially_overlapping = assignment_pair.iter()
            .filter(|p| p.partial_overlap())
            .count();
        println!("{}", count_partially_overlapping.to_string().bright_yellow());
    }
}
...
impl AssignmentPair {
    ...
    fn full_overlap(&self) -> bool {
        AssignmentPair::fully_contains(&self.left, &self.right) || AssignmentPair::fully_contains(&self.right, &self.left)
    }
